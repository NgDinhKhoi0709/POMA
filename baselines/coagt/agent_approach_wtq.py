...
path = 'dataset_wtq/data_01.jsonl'
output_file = f"outputs/result_wikiqa_data_01.jsonl"

correct = 0
t_samples = 0
empty_error_ids = []

with open(path, encoding='utf-8') as f1, open(output_file, "w", encoding="utf-8") as outfile:
    ...
    for i, l in tqdm(enumerate(f1)):
        ...
        first_half_rows=table[0:int(total_rows/2)]
        second_half_rows=table[int(total_rows/2):]
        ...
        chunks = chunk_table(table, max_tokens=2_000)
        ...
        for i, subtable in enumerate(chunks):
            ...
            k = prompt_agent_follow(title,question, headers, total_rows, agents_outputs[i], subtable, i+1,row_rank)
            k1 = get_completion(k,temperature=0.2, n=1)
        ...
        result=prompt_agent_synthesis(title,question, headers, total_rows, agents_outputs)
        response = get_completion(result,temperature=0.5,n=1)
        print(' agent_synthesis output  ====')
        print(response)

        ############### agent refiner #########################
        print("\n --- answer_refiner ---")
        # output_ans = re.findall(r"#(.*?)#", response)
        answer_final=prompt_answer_refiner(question,response)
        answer_final=get_completion(answer_final, temperature=0.5, n=1)
        print("ANSWER:",answer_final)
        str_output=answer_final
        # ################## evaluation ###################################

        print('\n final_output: ', str_output.lower(), 'Gold: ', answer)

        prediction = {'key': ids, 'question': question, 'response': response, 'prediction': str_output,
                      'answer': answer}
        outfile.write(json.dumps(prediction, ensure_ascii=False) + "\n")

        ## This is not official evaluation. .....
        if str_output.strip() == answer or str_output.strip().find(answer) != -1 \
                or answer.strip().find(str_output.strip()) != -1:
            correct += 1
...
# 2. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, with details such as town name, county, established and disestablished years, current status, and remarks.
#    - The counties listed in the subtable are: Shawnee County, Osage County, Lyon County, Wabaunsee County, Geary County, Morris County, Cherokee County, Labette County, Neosho County, Allen County, Woodson County, Wilson County, Elk County, Chautauqua County, and Butler County.
#
# 3. **Identifying Relevant Information:**
#    - Since the question is about ghost towns in the same county as "Chetola," we need to know the county where Chetola is located. Unfortunately, the subtable does not provide information about Chetola or its county.
#
# 4. **Previous Agent's Output:**
#    - The output from AGENT_FOL_1 indicates that they were unable to determine the county of Chetola because it was not present in their subtable.
#
# 5. **Conclusion:**
#    - Without knowing the county of Chetola, we cannot directly answer the question based on the current subtable.
#
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_1):**
#   - Unable to determine which ghost towns are in the same county as Chetola based on the current subtable.
#
# - **Present Answer (Your Answer):**
#   - Unable to determine which ghost towns are in the same county as Chetola based on the current subtable.
#
# - **Information for AGENT_FOL_3:**
#   - AGENT_FOL_3 should look for the county where Chetola is located in their subtable.
#   - Once Chetola's county is identified, AGENT_FOL_3 can compare it with the counties listed in their subtable to find ghost towns in the same county.
#   - The counties listed in this subtable are: Shawnee County, Osage County, Lyon County, Wabaunsee County, Geary County, Morris County, Cherokee County, Labette County, Neosho County, Allen County, Woodson County, Wilson County, Elk County, Chautauqua County, and Butler County.
#
# By following these steps, AGENT_FOL_3 can continue the analysis to answer the question about ghost towns in the same county as Chetola.
#
# --- Subtable 3 (30 rows) ---
# AGENT_COL_3
# : ### Step-by-Step Analysis
#
# 1. **Understanding the Question:**
#    - The task is to identify ghost towns in the same county as "Chetola."
#
# 2. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The relevant row for "Chetola" is present in this subtable: `['chetola', 'ellis county', '1886', '1907', 'townsite is now on private property.', '']`.
#
# 3. **Identifying Chetola's County:**
#    - From the subtable, we can see that "Chetola" is located in Ellis County.
#
# 4. **Finding Ghost Towns in the Same County:**
#    - Now that we know Chetola is in Ellis County, we need to identify any other ghost towns in Ellis County from the subtable.
#    - Another town listed in Ellis County in the subtable is "Rome": `['rome', 'ellis county', '1867', '1868', 'a marker near fort hays state college indicates where rome once was.', 'rome was co-founded by william "buffalo bill" cody.']`.
#
# 5. **Conclusion:**
#    - Based on the subtable, the ghost towns in the same county as Chetola (Ellis County) are "Chetola" and "Rome."
#
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_2):**
#   - Unable to determine which ghost towns are in the same county as Chetola based on the current subtable.
#
# - **Present Answer (Your Answer):**
#   - The ghost towns in the same county as Chetola (Ellis County) are "Chetola" and "Rome."
#
# - **Information for AGENT_FOL_4:**
#   - Chetola is located in Ellis County.
#   - The subtable identifies "Rome" as another ghost town in Ellis County.
#   - AGENT_FOL_4 should verify if there are any additional ghost towns in Ellis County in their subtable to provide a comprehensive answer.
#
# --- Subtable 4 (26 rows) ---
# AGENT_COL_4
# : ### Step-by-Step Analysis
...
#    - The task is to identify ghost towns in the same county as "Chetola."
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_3 identified that "Chetola" is located in Ellis County and found "Rome" as another ghost town in Ellis County.
#
# 3. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The subtable does not contain any towns from Ellis County. The counties listed in the subtable are Finney, Kearny, Hamilton, Grant, Haskell, Seward, Doniphan, Leavenworth, Wyandotte, Brown, Nemaha, Pottawatomie, Marshall, and Riley.
#
# 4. **Conclusion:**
#    - Based on the subtable I have, there are no additional ghost towns in Ellis County. Therefore, I cannot add any new information about ghost towns in the same county as Chetola from my subtable.
#
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_3):**
#   - The ghost towns in the same county as Chetola (Ellis County) are "Chetola" and "Rome."
#
# - **Present Answer (Your Answer):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Information for AGENT_FOL_5:**
#   - Chetola is located in Ellis County.
#   - AGENT_FOL_3 identified "Rome" as another ghost town in Ellis County.
#   - My subtable does not contain any towns from Ellis County, so AGENT_FOL_5 should check their subtable for any additional ghost towns in Ellis County to provide a comprehensive answer.
#
# --- Subtable 5 (22 rows) ---
# AGENT_COL_5
# : ### Step-by-Step Analysis
...
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_4 confirmed that "Chetola" is located in Ellis County and identified "Rome" as another ghost town in Ellis County.
#    - AGENT_FOL_4's subtable did not contain any towns from Ellis County.
#
# 3. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The counties listed in my subtable are Johnson, Linn, Douglas, Shawnee, Wabaunsee, Osage, Coffey, Lyon, Morris, Bourbon, Neosho, Crawford, and Cherokee.
#    - There are no towns from Ellis County in my subtable.
#
# 4. **Conclusion:**
#    - Based on the subtable I have, there are no additional ghost towns in Ellis County. Therefore, I cannot add any new information about ghost towns in the same county as Chetola from my subtable.
#
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_4):**
#   - The ghost towns in the same county as Chetola (Ellis County) are "Chetola" and "Rome."
#
# - **Present Answer (Your Answer):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Information for AGENT_FOL_6:**
#   - Chetola is located in Ellis County.
#   - AGENT_FOL_3 identified "Rome" as another ghost town in Ellis County.
#   - My subtable does not contain any towns from Ellis County, so AGENT_FOL_6 should check their subtable for any additional ghost towns in Ellis County to provide a comprehensive answer.
#
# --- Subtable 6 (29 rows) ---
# AGENT_COL_6
# : ### Step-by-Step Analysis
...
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_5 confirmed that "Chetola" is located in Ellis County.
#    - AGENT_FOL_4 identified "Rome" as another ghost town in Ellis County.
#    - AGENT_FOL_5's subtable did not contain any towns from Ellis County.
#
# 3. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The counties listed in my subtable are Montgomery, Chautauqua, Wilson, Woodson, Greenwood, Butler, Washington, Jewell, Mitchell, Cloud, Dickinson, Lincoln, Ellsworth, Barton, Rice, Ellis, Rush, Sumner, and Harper.
#    - Notably, my subtable includes a town from Ellis County: "Yocemento."
#
# 4. **Conclusion:**
#    - Based on the subtable I have, "Yocemento" is an additional ghost town in Ellis County. Therefore, it is in the same county as "Chetola."
#
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_5):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Present Answer (Your Answer):**
#   - In addition to "Chetola" and "Rome," "Yocemento" is another ghost town in Ellis County.
#
# - **Information for AGENT_FOL_7:**
#   - Chetola is located in Ellis County.
#   - AGENT_FOL_4 identified "Rome" as another ghost town in Ellis County.
#   - My subtable contains "Yocemento" as an additional ghost town in Ellis County.
#   - AGENT_FOL_7 should verify if there are any more ghost towns in Ellis County in their subtable to provide a comprehensive answer.
#
# --- Subtable 7 (32 rows) ---
# AGENT_COL_7
# : ### Step-by-Step Analysis
...
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_6 confirmed that "Chetola" is located in Ellis County.
#    - AGENT_FOL_4 identified "Rome" as another ghost town in Ellis County.
#    - AGENT_FOL_6's subtable included "Yocemento" as an additional ghost town in Ellis County.
#
# 3. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The counties listed in my subtable are Harper, Sedgwick, Kingman, Reno, Barber, Comanche, Pratt, Stafford, Edwards, Norton, Decatur, Rawlins, Sherman, Lane, Wallace, Finney, Clark, Seward, Stevens, and Stanton.
#    - Notably, my subtable does not contain any towns from Ellis County.
#
# 4. **Conclusion:**
#    - Based on the subtable I have, there are no additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_6):**
#   - In addition to "Chetola" and "Rome," "Yocemento" is another ghost town in Ellis County.
#
# - **Present Answer (Your Answer):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Information for AGENT_FOL_8:**
#   - Chetola is located in Ellis County.
#   - AGENT_FOL_4 identified "Rome" as another ghost town in Ellis County.
#   - AGENT_FOL_6's subtable contained "Yocemento" as an additional ghost town in Ellis County.
#   - My subtable does not contain any towns from Ellis County.
#   - AGENT_FOL_8 should verify if there are any more ghost towns in Ellis County in their subtable to provide a comprehensive answer.
#
# --- Subtable 8 (26 rows) ---
# AGENT_COL_8
# : ### Step-by-Step Analysis
#
# 1. **Understanding the Question:**
#    - The task is to identify ghost towns in the same county as "Chetola," which is located in Ellis County.
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_7 confirmed that "Chetola" is located in Ellis County.
#    - "Rome" and "Yocemento" are other ghost towns in Ellis County, as identified by previous agents.
#
# 3. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The counties listed in my subtable are Atchison, Leavenworth, Jefferson, Nemaha, Marshall, Riley, Pottawatomie, Johnson, Miami, Anderson, Franklin, Douglas, Coffey, and Osage.
#    - Notably, my subtable does not contain any towns from Ellis County.
#
# 4. **Conclusion:**
#    - Based on the subtable I have, there are no additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
...
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_7):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Present Answer (Your Answer):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Information for AGENT_FOL_9:**
#   - Chetola is located in Ellis County.
#   - "Rome" and "Yocemento" are other ghost towns in Ellis County.
#   - My subtable does not contain any towns from Ellis County.
#   - AGENT_FOL_9 should verify if there are any more ghost towns in Ellis County in their subtable to provide a comprehensive answer.
#
# --- Subtable 9 (27 rows) ---
# AGENT_COL_9
# : ### Step-by-Step Analysis
...
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_8 confirmed that "Chetola" is located in Ellis County.
#    - "Rome" and "Yocemento" are other ghost towns in Ellis County, as identified by previous agents.
#    - AGENT_FOL_8's subtable did not contain any towns from Ellis County.
#
# 3. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The counties listed in my subtable are Shawnee, Wabaunsee, Geary, Lyon, Republic, Smith, Ottawa, Dickinson, Marion, Saline, Ellsworth, Russell, Barton, Rice, McPherson, Sedgwick, and Kingman.
#    - Notably, my subtable does not contain any towns from Ellis County.
#
# 4. **Conclusion:**
#    - Based on the subtable I have, there are no additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
...
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_8):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Present Answer (Your Answer):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Information for AGENT_FOL_10:**
#   - Chetola is located in Ellis County.
#   - "Rome" and "Yocemento" are other ghost towns in Ellis County.
#   - My subtable does not contain any towns from Ellis County.
#   - AGENT_FOL_10 should verify if there are any more ghost towns in Ellis County in their subtable to provide a comprehensive answer.
#
# --- Subtable 10 (32 rows) ---
# AGENT_COL_10
# : ### Step-by-Step Analysis
...
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_9 confirmed that "Chetola" is located in Ellis County.
#    - "Rome" and "Yocemento" are other ghost towns in Ellis County, as identified by previous agents.
#    - AGENT_FOL_9's subtable did not contain any towns from Ellis County.
#
# 3. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The counties listed in my subtable are Sumner, Pratt, Kiowa, Norton, Phillips, Sherman, Decatur, Trego, Logan, Wallace, Gove, Finney, Gray, Hodgeman, Haskell, Stevens, Morton, Meade, Stanton, Harvey, Chase, Wichita, and Linn.
#    - Notably, my subtable does not contain any towns from Ellis County.
#
# 4. **Conclusion:**
#    - Based on the subtable I have, there are no additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
...
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_9):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Present Answer (Your Answer):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Information for AGENT_FOL_11:**
#   - Chetola is located in Ellis County.
#   - "Rome" and "Yocemento" are other ghost towns in Ellis County.
#   - My subtable does not contain any towns from Ellis County.
#   - AGENT_FOL_11 should verify if there are any more ghost towns in Ellis County in their subtable to provide a comprehensive answer.
#
# --- Subtable 11 (34 rows) ---
# AGENT_COL_11
# : ### Step-by-Step Analysis
...
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_10 confirmed that "Chetola" is located in Ellis County.
#    - "Rome" and "Yocemento" are other ghost towns in Ellis County, as identified by previous agents.
#    - AGENT_FOL_10's subtable did not contain any towns from Ellis County.
#
# 3. **Subtable Analysis:**
#    - The subtable provided contains information about various ghost towns in Kansas, including details such as town name, county, established and disestablished years, current status, and remarks.
#    - The counties listed in my subtable are Graham, Douglas, Ellis, Cloud, Chase, Wyandotte, Jefferson, Johnson, Anderson, Franklin, Shawnee, Geary, Chautauqua, Elk, Allen, Clay, Mitchell, Saline, Ellsworth, Rush, Barton, Rice, McPherson, Reno, Sedgwick, Kingman, Barber, Meade, Edwards, and Clark.
#    - Notably, my subtable contains one town from Ellis County: "Smoky Hill City."
#
# 4. **Conclusion:**
#    - Based on the subtable I have, "Smoky Hill City" is an additional ghost town in Ellis County. Therefore, it is in the same county as Chetola.
#
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_10):**
#   - My subtable does not contain any additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Present Answer (Your Answer):**
#   - My subtable contains "Smoky Hill City," which is a ghost town in Ellis County. Therefore, it is in the same county as Chetola.
#
# - **Information for AGENT_FOL_12:**
#   - Chetola is located in Ellis County.
#   - "Rome," "Yocemento," and "Smoky Hill City" are ghost towns in Ellis County.
#   - AGENT_FOL_12 should verify if there are any more ghost towns in Ellis County in their subtable to provide a comprehensive answer.
#
# --- Subtable 12 (1 rows) ---
# AGENT_COL_12
# : ### Step-by-Step Analysis
...
#
# 2. **Reviewing Previous Analysis:**
#    - AGENT_FOL_11 confirmed that "Chetola" is located in Ellis County.
#    - "Rome," "Yocemento," and "Smoky Hill City" are other ghost towns in Ellis County, as identified by previous agents.
#
# 3. **Subtable Analysis:**
#    - My subtable contains only one row with the following information:
#      - Town Name: Croft
#      - County: Pratt County
#      - Established: Not specified
#      - Disestablished: 2000s
#      - Current Status: Two vacant grain elevators, two vacant houses, an old school, and a few ruins remain in this town.
#      - Remarks: None provided
#    - The town "Croft" is located in Pratt County, not Ellis County.
#
# 4. **Conclusion:**
#    - Based on the information in my subtable, there are no additional ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# ### Outputs
#
# - **Previous Answer (AGENT_FOL_11):**
#   - My subtable contains "Smoky Hill City," which is a ghost town in Ellis County. Therefore, it is in the same county as Chetola.
#
# - **Present Answer (Your Answer):**
#   - My subtable does not contain any ghost towns in Ellis County. Therefore, I cannot provide further information on ghost towns in the same county as Chetola.
#
# - **Information for AGENT_FOL_13:**
#   - Chetola is located in Ellis County.
#   - "Rome," "Yocemento," and "Smoky Hill City" are ghost towns in Ellis County.
#   - My subtable does not contain any additional ghost towns in Ellis County.
#   - AGENT_FOL_13 should verify if there are any more ghost towns in Ellis County in their subtable to provide a comprehensive answer.
#
#  --- agent_synthesis ---
#
#  --- Collecte outputs, analyse, synthesis ---
#  agent_synthesis output  ====
# ### Step-by-Step Analysis
#
# 1. **Understanding the Question:**
#    - We need to identify ghost towns in the same county as "Chetola," which is located in Ellis County.
#
# 2. **Reviewing AGENTS' Outputs:**
#    - **AGENT_COL_3:** Identified "Chetola" and "Rome" as ghost towns in Ellis County.
#    - **AGENT_COL_6:** Added "Yocemento" as another ghost town in Ellis County.
#    - **AGENT_COL_11:** Identified "Smoky Hill City" as an additional ghost town in Ellis County.
#    - Other agents confirmed the presence of these towns and noted that their subtables did not contain any additional ghost towns from Ellis County.
#
# 3. **Collecting Information:**
#    - We have four ghost towns identified in Ellis County: "Chetola," "Rome," "Yocemento," and "Smoky Hill City."
#
# 4. **Review the Question:**
#    - The question asks for ghost towns in the same county as Chetola, which we now know is Ellis County.
#
# 5. **Final Answer:**
#    - The ghost towns in the same county as Chetola (Ellis County) are "Chetola," "Rome," "Yocemento," and "Smoky Hill City."
#
#  --- answer_refiner ---
# 1it [01:15, 75.36s/it]
# ANSWER: "rome, yocemento, smoky hill city"
#
#  final_output:  "rome, yocemento, smoky hill city" Gold:  rome, yocemento, smoky hill city
# correct:  1
