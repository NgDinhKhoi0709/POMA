...
#let header(self, dpt-color: none) = {
  ...
  block(
    ...
    inset: (left: 1em), {
      ...
      // Right-hand header cell is intentionally empty: this deck carries no
      // institutional logo. The invisible strut preserves the row height the
      // logo used to set, so the nav rows stay vertically centred instead of
      // riding against the top edge. Restore a logo by replacing the box.
      show: it => grid(
        columns: (1fr, auto),
...
#let footer(self) = {
  ...
  // Two cells only: the short title flush left, the slide counter flush right.
  // The old middle "mini-authors" cell was removed on purpose so the footer
  // carries no personal name or affiliation.
  grid(
    columns: (1fr, auto),
    column-gutter: 1em,
    align: (start + horizon, end + horizon),
